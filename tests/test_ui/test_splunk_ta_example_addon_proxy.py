...
TA_PROXY_URL = (
    "/servicesNS/nobody/Splunk_TA_UCCExample/splunk_ta_uccexample_settings/proxy"
)
...
def reset_configuration(ucc_smartx_rest_helper):
    yield
    proxy = Proxy(
        TA_NAME, TA_PROXY_URL, TA_CONF, ucc_smartx_rest_helper=ucc_smartx_rest_helper
    )
...
class TestProxy(UccTester):
    ...
    @pytest.mark.proxy
    def test_proxy_fields_label_entity(
    ...
    def test_proxy_host_field_length_validation(
        ...
        self.assert_util(
            proxy.save, "Max host length is 4096", left_args={"expect_error": True}
        )
    ...
    def test_proxy_port_field_valid_range(
        ...
        self.assert_util(
            proxy.save, "Field Port is not a number", left_args={"expect_error": True}
        )

    @pytest.mark.execute_enterprise_cloud_false
    @pytest.mark.forwarder
    @pytest.mark.proxy
    def test_proxy_port_field_out_of_range(
        self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper
    ):
    ...
    @pytest.mark.forwarder
    @pytest.mark.proxy
    def test_proxy_list_proxy_types(
        self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper
    ):
    ...
    @pytest.mark.forwarder
    @pytest.mark.proxy
    def test_proxy_required_field_proxy_type(
        self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper
    ):
        ...
        proxy.type.cancel_selected_value()
        self.assert_util(
            proxy.save, "Proxy type can not be empty", left_args={"expect_error": True}
        )
    ...
    def test_proxy_username_field_length_validation(
        ...
        self.assert_util(
            proxy.save, "Max length of username is 50", left_args={"expect_error": True}
        )
    ...
    def test_proxy_frontend_validation(
        self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper
    ):
    ...
    @pytest.mark.proxy
    @pytest.mark.sanity_test
    def test_proxy_backend_validation(
        self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper
    ):
        """Verifies the proxy is saved properly in frontend after saving it"""
        proxy = Proxy(
