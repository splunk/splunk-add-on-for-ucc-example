...
from pytest_splunk_addon_ui_smartx.alert_actions.components.checkbox import AlertCheckbox
from pytest_splunk_addon_ui_smartx.alert_actions.components.single_select import AlertSingleSelect
from pytest_splunk_addon_ui_smartx.alert_actions.components.toggle import AlertToggle
from pytest_splunk_addon_ui_smartx.alert_actions.components.account_select import AlertAccountSelect
...
class AlertPage(Page):
    def __init__(self, ucc_smartx_selenium_helper, ucc_smartx_rest_helper, open_page=True):
        super(AlertPage, self).__init__(ucc_smartx_selenium_helper, ucc_smartx_rest_helper, open_page)
        if ucc_smartx_selenium_helper:
            self.alert_table = AlertTable(ucc_smartx_selenium_helper.browser)
            self.alert_entity = AlertEntity(ucc_smartx_selenium_helper, ucc_smartx_rest_helper)
            self.action_entity = TestAction(ucc_smartx_selenium_helper.browser)
        if ucc_smartx_rest_helper:
            self.backend_conf = ListBackendConf(self._get_alert_endpoint(), ucc_smartx_rest_helper.username, ucc_smartx_rest_helper.password)
    ...
    def open(self):
        ...
        print("HERE I AM")
        print(self.splunk_web_url + "en-US/manager/Splunk_TA_UCCExample/saved/searches")
