...
class Custom(Entity):
    ...
    def __init__(self, ucc_smartx_configs):
        ...
        # Components
        self.test_string = TextBox(browser, Selector(select=".test_string"))
        self.test_number = TextBox(browser, Selector(select=".test_number"))
        self.test_regex = TextBox(browser, Selector(select=".test_regex"))
        self.test_email = TextBox(browser, Selector(select=".test_email"))
        self.test_ipv4 = TextBox(browser, Selector(select=".test_ipv4"))
        self.test_date = TextBox(browser, Selector(select=".test_date"))
        self.test_url = TextBox(browser, Selector(select=".test_url"))
