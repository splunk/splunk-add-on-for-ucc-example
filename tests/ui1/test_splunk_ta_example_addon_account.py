...
class TestAccount(UccTester):
    ...
    def test_account_delete_valid_prompt_message(
        ...
        self.assert_util(
            prompt_message,
            'Are you sure you want to delete "{}" ? Ensure that no input is configured with "{}" as this will stop data collection for that input.'.format(
                ACCOUNT_CONFIG["name"], ACCOUNT_CONFIG["name"]
            ),
        )
